#!/usr/bin/env python3
"""
Test runner for API Gateway integration tests
Provides comprehensive test execution for dual routing path validation
"""
...
class APIGatewayIntegrationTestRunner:
    """Custom test runner for API Gateway integration tests"""
    ...
    def run_tests_with_coverage(self):
        ...
        print("=" * 80)
        print("API GATEWAY DUAL ROUTING - INTEGRATION TEST EXECUTION")
        print("=" * 80)
        ...
        try:
            # Import test modules
            from test_api_gateway_integration import (
            ...
            print(f"Discovered {total_tests} integration test cases across {len(test_classes)} test classes")
            print()
    ...
    def _generate_coverage_report(self, cov):
        ...
        try:
            ...
            try:
                html_dir = os.path.join(os.path.dirname(__file__), 'coverage_html_integration')
    ...
    def _print_summary(self):
        ...
        print("TEST CATEGORIES EXECUTED:")
        print("-" * 40)
        print("✓ API Gateway path routing validation")
        print("✓ Internet vs VPN Lambda routing detection")
        print("✓ Authentication consistency across paths")
        print("✓ GET request routing (models and info)")
        print("✓ Request context preservation")
        print("✓ Error response consistency")
        print("✓ Response header validation")
        ...
        if self.results['failed_tests'] == 0 and self.results['error_tests'] == 0:
            ...
            print("✓ API Gateway dual routing is working correctly")
            print("✓ Both internet and VPN paths are properly configured")
            print("✓ Authentication is consistent across routing methods")
            print("✓ Error handling is uniform between Lambda functions")
        else:
            print("❌ SOME INTEGRATION TESTS FAILED")
            print("⚠ API Gateway routing may have issues - Review details above")
        ...
        try:
            report_data = {
                'test_run': {
                    ...
                    'component': 'api_gateway_integration',
                ...
                'test_categories': {
                    'path_routing': 'API Gateway path routing validation',
                    'lambda_routing': 'Internet vs VPN Lambda routing detection',
                    'authentication': 'Authentication consistency across paths',
                    'get_requests': 'GET request routing validation',
                    'context_preservation': 'Request context preservation',
                    'error_consistency': 'Error response consistency',
                    'header_validation': 'Response header validation',
                    'path_parameters': 'Path parameter extraction',
                    'invalid_paths': 'Invalid path handling',
                    'query_parameters': 'Query parameter preservation'
                },
            ...
            # Write JSON report
            report_path = os.path.join(os.path.dirname(__file__), 'api_gateway_integration_test_results.json')
            with open(report_path, 'w') as f:
                json.dump(report_data, f, indent=2)
            
            print(f"📊 JSON integration test report generated: {report_path}")
...
def main():
    ...
    runner = APIGatewayIntegrationTestRunner()
